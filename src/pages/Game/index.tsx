...
const Game: React.FC = () => {
  ...
  const favouriteItem = useSelector<RootState>((state) =>
    state.favourites.items.find((obj) => obj.id === id)
  );
    ...
    async function fetchGames() {
      ...
      try {
        const { data } = await axios.get(
          `https://6516e83f09e3260018ca764a.mockapi.io/items/${urlID}`
        );
        setGame(data);
        setIsLoading(false);
      } catch (error) {
          ...
          <div className={styles.game__info}>
            <div className={styles.game__headlineBlock}>
              ...
              <svg
                ...
                className={`${
                  !favouriteItem
                    ? styles.game__add_to_favourite
                    : styles.game__add_to_favourite_active
                }`}
                width="40px"
                height="40px"
                viewBox="0 0 24 24"
            ...
            <Link
              ...
              className={`${
                cartItem
                  ? styles.game__add_to_cart_active
                  : styles.game__add_to_cart
              }`}
            >
              {cartItem ? "В корзинe" : "В корзину"}
            </Link>
          </div>
        </>
      )}
